...
void Update()
{
    float minTheta = DEG2RAD(10.0);
    float maxTheta = DEG2RAD(170.0);
    ...
    eyeX = radius * sin(theta) * cos(phi);
    eyeY = radius * cos(theta);
    eyeZ = radius * sin(theta) * sin(phi);
}
...
int main()
{
    ...
    window = glfwCreateWindow(width, height, "Laboratory 1", NULL, NULL);
    if (!window)
    {
